...
export function QuotationGenerator({ currency, companyDetails, bankDetails, showCompanyTaxId, idService }: QuotationGeneratorProps) {
  ...
  const [notes, setNotes] = useState('This quotation is valid for 15 days.');
  ...
  useEffect(() => {
    ...
    setQuotationNumber(idService.getNextId('quotation'));
  ...
  const generatePDF = () => {
    ...
    doc.setFontSize(20);
    doc.text("Quotation", doc.internal.pageSize.getWidth() - 14, 22, { align: 'right' });

    doc.setFontSize(12);
    doc.text(`Quotation #: ${quotationNumber}`, doc.internal.pageSize.getWidth() - 14, 32, { align: 'right' });
    ...
    doc.text(fromLines, 14, 50);
     ...
     let toLines = [
        ...
        selectedCustomer.taxId ? `Tax ID: ${selectedCustomer.taxId}` : ''
    ].filter(Boolean);
    doc.text(toLines, 120, 50);
    ...
    doc.text(`Tax (${taxRate}%): ${formatCurrency(taxAmount)}`, 140, finalY + 15);
    doc.setFontSize(14);
        ...
        <CardContent className="space-y-6">
            ...
            <CardContent className="space-y-4">
                <div className="flex items-end gap-2">
                    <div className="flex-grow space-y-2">
                      <Label htmlFor="customer-select-qt">Select Customer</Label>
                      <Select onValueChange={setSelectedCustomerId} value={selectedCustomerId || ''}>
                          <SelectTrigger id="customer-select-qt">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex items-center space-x-2">
                    <Label htmlFor="show-discounts-qt">Show Discounts</Label>
                    <Switch id="show-discounts-qt" checked={showDiscounts} onCheckedChange={setShowDiscounts} />
                </div>
            </div>
                    ...
                    <TableRow key={item.id}>
                        ...
                        <TableCell>
                            <Button asChild variant="outline" size="sm" className="w-full h-16 relative">
                                <label htmlFor={`img-upload-qt-${index}`} className="cursor-pointer flex items-center justify-center">
                                    {item.image ? <img src={item.image} alt="upload" className="object-cover w-full h-full" /> : <Upload className="h-4 w-4" />}
                                    <input id={`img-upload-qt-${index}`} type="file" accept="image/*" className="sr-only" onChange={(e) => handleImageUpload(index, e)}/>
                                </label>
                            </Button>
                        </TableCell>
                        <TableCell>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
            ...
            <div className="space-y-2">
                ...
                <Textarea value={paymentTerms} onChange={e => setPaymentTerms(e.target.value)} placeholder="e.g., 50% advance, 50% on delivery." />
            </div>
            <div className="space-y-2 md:col-span-2">
                <Label>Notes / Additional Information</Label>
                <Textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="e.g., This quotation is valid for 15 days." />
            </div>
          </div>
        </CardContent>
      </Card>
          ...
          <CardContent>
            <div className="p-6 border rounded-lg bg-muted/20 space-y-4">
                 <div className="flex justify-between items-start">
                    ...
                    <h3 className="text-xl font-bold text-right">QUOTATION</h3>
                </div>
                <div className="flex justify-between text-sm">
                 ...
                 <div className="grid grid-cols-2 gap-4 text-xs">
                    ...
                    <div className="text-sm text-right">
                        <p className="font-bold">To: {selectedCustomer?.name || "..."}</p>
