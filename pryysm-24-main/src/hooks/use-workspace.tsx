...
import { generateInitialUnassignedJobs, generateInitialOrders, generateInitialCustomers, generateInitialDocuments, generateInitialInventory, generateInitialRawMaterials, generateInitialPrinters, initialScheduleData, generateInitialCostingTemplates, generateInitialLoggedCalculations } from "@/hooks/workspace-data";
...
type WorkspaceContextType = Omit<WorkspaceData, 'idCounters'> & {
    ...
    getAvailableMaterialNames: (technology: string, material: string, finish: string) => { name: string, color: string, stock: number }[];
    ...
    if (forNewUser) {
        return {
            ...
            featureFlags: {
                ...
                Pro: { tabs: allAvailableFeatures, printerLimits: { 'FDM': 20, 'SLA': 5, 'SLS': 2, 'DLP': 0, 'MJF': 0, 'EBM': 0, 'DMLS': 0 } },
...
export function WorkspaceProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        const loadData = async () => {
            if (isAuthenticated && user?.email) {
                ...
                if (isDemoUser) {
                    setWorkspaceState(getDefaultWorkspaceData(false)); // Always load fresh demo data
                } else if (isNewSignup) {
                ...
                // This will use placeholder images by default on first load.
                // Dynamic generation was removed in the previous step.
    ...
    // Dummy setter for landing page content to satisfy type, logic is removed
    const setLandingPageContent = useCallback((updater: React.SetStateAction<LandingPageContent | null>) => {
        // This function is now a no-op as dynamic image generation was reverted.
        // It's kept for type consistency but does not change state.
    }, []);
    ...
    const idService = useMemo(() => ({
        getNextId: (type: keyof CodeSettings) => {
            const prefix = codeSettings[type] || `${type.toUpperCase()}-`;
            let nextId = 0;
    ...
    const confirmJobUpload = useCallback((jobId: string | number, printerId: string) => {
        ...
        setWorkspaceState(prev => {
            ...
            const newSchedule = prev.schedule.map(printerSchedule => {
                if (printerSchedule.printerId === printerId) {
                    const newJobs = printerSchedule.jobs.map(job => {
                        if (job.id === jobId) {
                            jobName = job.name;
                            printerName = prev.printers.find(p => p.id === printerId)?.name || printerId;
                            return { ...job, isConfirmed: true, color: getColorForDuration(job.duration) };
                        }
                        return job;
                    });
                    return { ...printerSchedule, jobs: newJobs };
                }
        ...
        if (jobName && printerName) {
            toast({
                title: 'Job Confirmed',
                description: `Job "${jobName}" on printer "${printerName}" has been confirmed.`
            });
        }
    }, [toast]);
    ...
    const assignJobToMachine = (job: Job, machineId: string, parentJobId?: string | number) => {
        setWorkspaceState(prev => {
            ...
            const machine = prev.printers.find(m => m.id === machineId);
            if (!machine) {
                toast({ title: "Error", description: "Machine not found.", variant: "destructive" });
                return prev;
            }

            const printerSchedule = prev.schedule.find(s => s.printerId === machineId);
            ...
            const isSubItem = parentJobId !== undefined;
            const sourceJob = prev.unassignedJobs.find(j => j.id === (isSubItem ? parentJobId : job.id));
            if (!sourceJob) {
                toast({ title: "Error", description: "Original job not found in queue.", variant: "destructive" });
                return prev;
            }

            const durationHours = isSubItem
                ? (sourceJob.estimatedTime || 0) / sourceJob.items / 60
                : (job.estimatedTime || 0) / 60;
            
            const endTime = addHours(startTime, durationHours);

            const newJob: Job = {
                ...job,
                id: isSubItem ? `${parentJobId}-item-${uid('sub')}` : job.id,
                name: isSubItem ? `${sourceJob.name} (Item)` : job.name,
                items: 1,
            ...
            const newSchedule = prev.schedule.map(s => {
                if (s.printerId === machineId) {
                    return { ...s, jobs: [...s.jobs, newJob] };
                }
                return s;
            });
            
            const newUnassignedJobs = prev.unassignedJobs.map(j => {
                if (j.id === parentJobId) {
                    const newItems = j.items - 1;
                    if (newItems <= 0) return null; // Mark for removal
                    return { ...j, items: newItems };
                }
                if (!isSubItem && j.id === job.id) return null; // Mark for removal
                return j;
            ...
            toast({
                ...
                description: `Check "3D Printer Management" to confirm file upload for "${newJob.name}".`
            });

            return { ...prev, schedule: newSchedule, unassignedJobs: newUnassignedJobs };
        });
    };
    ...
    const getAvailableMaterialNames = useCallback((technology: string, material: string, finish: string): { name: string, color: string, stock: number }[] => {
        if (!material || !finish) return [];
        const relevantMaterials = getRelevantMaterials(technology);
        const nameStock: { [name: string]: { color: string, stock: number } } = {};
        
        relevantMaterials
            .filter(item => 
                item &&
                ((item as any).material || (item as any).type) === material && 
                ((item as any).finish || 'Standard') === finish && 
                (item as any).status !== 'Empty'
            )
            .forEach(item => {
                const nameKey = (item as any).name;
                if (nameKey) {
                    if (!nameStock[nameKey]) {
                        nameStock[nameKey] = { color: (item as any).color || '#000000', stock: 0 };
                    }
                    nameStock[nameKey].stock++;
                }
            });

        return Object.entries(nameStock).map(([name, data]) => ({ name, color: data.color, stock: data.stock }));
    }, [getRelevantMaterials]);
    
    const getMaterialColor = useCallback((materialName: string): string | undefined => {
        if (!workspaceState) return undefined;
        const allMaterials = [...workspaceState.spools, ...workspaceState.resins, ...workspaceState.powders];
    ...
    const value: WorkspaceContextType = {
        ...
        getAvailableMaterialsForTechnology, getAvailableFinishesForMaterial, getAvailableMaterialNames, getMaterialColor,
